...
export default function DashboardPage() {
  ...
  const { data: borrowings = [] } = useListBorrowings();
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div variants={container} initial="hidden" animate="show"
        ...
        <StatCard icon={Clock} label="Menunggu Approval" value={borrowings.filter((borrowing) => borrowing.status === "pending").length}
          colorClass="bg-blue-500/12 text-blue-600 dark:text-blue-400" glowClass="card-glow-blue" />
      ...
      <motion.div
        ...
        className="flex flex-wrap items-center gap-2 rounded-2xl border border-border/70 bg-background/60 p-3"
      >
        <span className="mr-1 text-xs font-bold text-muted-foreground">Aksi cepat</span>
        <Link href="/borrowings" className="rounded-xl bg-primary/10 px-3 py-2 text-xs font-bold text-primary transition hover:bg-primary hover:text-primary-foreground">Review request</Link>
        <Link href="/borrowings" className="rounded-xl bg-primary/10 px-3 py-2 text-xs font-bold text-primary transition hover:bg-primary hover:text-primary-foreground">Input peminjaman</Link>
        <Link href="/books" className="rounded-xl bg-primary/10 px-3 py-2 text-xs font-bold text-primary transition hover:bg-primary hover:text-primary-foreground">Tambah buku</Link>
      </motion.div>
