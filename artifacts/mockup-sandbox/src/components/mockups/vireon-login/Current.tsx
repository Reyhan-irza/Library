 ...
 * Auth controls are inert in this isolated baseline; no credentials or live calls.
 */
...
const VIREON_LOGO = "/__mockup/vireon-logo-new.png";

type _LinkProps = ComponentProps<"a">;

function Link({ onClick, ...props }: _LinkProps) {
  return (
    <a
      {...props}
      onClick={(event) => {
...
function useLocation(): [null, (_path: string) => void] {
...
function useLogin() {
  return {
    ...
    mutate: (
      _input: unknown,
      _callbacks?: { onSuccess?: () => void; onError?: (error: unknown) => void },
    ) => undefined,
  };
}
